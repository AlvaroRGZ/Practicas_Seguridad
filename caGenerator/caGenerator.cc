...
string caGenerator(int satID, int lenght, bool trace){

  // Obtenemos los indices del satelite
  pair<int, int> x = selectorGPS(satID);
  int fb = 10 - x.first, sb = 10 - x.second; 
  ...
  for (int i = 0; i < lenght; i++) {
    ...
    re2 = (g2[8] + g2[7] + g2[4] + g2[2] + g2[1] + g2[0])%2;
    ca2 = (g2[fb] + g2[sb])%2;
    RDRL(g2);
    g2.set(9, re2); // Realimentamos con r2 por la cabeza

    output += bitset<1>((ca1 + ca2)%2).to_string();

    if (trace) {
      cout << "re1: " << re1 << " ca1: "<< ca1 << endl;
      cout << "sec = " << g1.to_string() << endl;
      cout << "re2: " << re2 << " ca2: "<< ca2 << endl;
      cout << "sec2 = " << g2.to_string() << endl;
      cout << "caBit = " << (ca1+ ca2)%2 << endl << endl;
    }
  }
  return output;
}
...
int main (void){
  ...
  while(!quit){
    ...
    case 1:{
      bool t;
      cout << "Trace? > "; cin >> t; cout << endl;
      cout << "Secuencia C/A PRN1 = " << caGenerator(1, 14, t) << endl;
      break;
    }
    case 0:
      quit = true;
